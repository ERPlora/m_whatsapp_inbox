...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("is_deleted", sa.Boolean(), server_default="false", nullable=False, index=True),
        sa.Column("deleted_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("employee_id", sa.Uuid(), sa.ForeignKey("accounts_localuser.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("assigned_to_id", sa.Uuid(), sa.ForeignKey("accounts_localuser.id", ondelete="SET NULL"), nullable=True),
